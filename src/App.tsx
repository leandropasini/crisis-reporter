...
export default function App({ mode }: Props) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const setHeight = () => {
      document.documentElement.style.setProperty("--demo-disclaimer-h", `${el.offsetHeight}px`);
    };
    setHeight();
    ...
    return () => {
      ...
      document.documentElement.style.removeProperty("--demo-disclaimer-h");
    };
  }, [mode]);

  return (
    <CrisisModeProvider>
      {/* Demo disclaimer banner — illustrative data notice, all viewports */}
      {mode === "demo" && (
        <div
          ref={bannerRef}
          style={{
